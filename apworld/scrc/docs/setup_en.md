# Super Crazy Rhythm Castle Archipelago setup — v0.20.0
...
5. Run RhythmCastleAP **v0.67.94** with `DirectStartAtPhoneHub=true`, `EnableAreaAccessPrototype=true`, and `PrototypeStartingArea=AP`; keep the retired `RandomizeEarlyProgression` prototype false.
...
v0.20.0 (`area-routing-plant-pipes-0.15-generation-foundation-0.16-hip-glasses-chicken-bucket-0.17-next-release-repair-0.18-consolidated-preview-0.19-difficulty-filtering-0.20`) forces Roots as the starter area, retains Weed Killer and cartridge randomization, and adds active difficulty filtering. Rebuild/replace the APWorld, restart Archipelago, and generate a fresh seed after APWorld changes. The unchanged v0.67.94 client remains compatible with existing v0.19 seeds, which retain their old location sets.
