...
Status: **experimental Client v0.69.0 / APWorld v0.23.0 candidate partially live-verified but not accepted for release**. Offline verification and scoped final review passed. Live testing verified weighted receipts, the 180-point cap, the upper four chest thresholds, all nine one-time chest checks, medal invariance, retained disconnected display, full-relaunch restoration, and automatic reconnect/history rebuild. A repeat diagnostic showed that the earlier reconnect failure was a 30-second backoff delay rather than a stopped worker. The reviewed responsiveness change and Game Garage entrance-preview fix were then deployed as the exact reviewed artifact and passed focused live regression: retry delays saturated at five seconds, reconnection restored exactly 180 points, and the entrance preview displayed only the genuinely available native Vampire Killer cartridge. The lower five threshold pairs, offline queued-check recovery, and compatibility regressions remain incomplete. The native chest detour candidate is rejected and must not be deployed.
...
**NOT ACCEPTED — remaining targeted live regression required.** The reviewed room-scoped implementation passed offline verification and the core weighted-point, cap, upper-threshold, one-time-check, medal-invariance, disconnected-display, automatic-reconnect/history-rebuild, and full-relaunch tests. The repeat reconnect diagnostic proved that the worker had not stopped; it was waiting at its prior 30-second maximum backoff. Reviewed commit `60987df` reduces that maximum wait to five seconds while preserving infinite retries and stale-worker protection; the exact reviewed DLL was deployed and live logs confirmed delays of 1, 2, 5, 5 seconds followed by automatic recovery and an unchanged 180-point rebuild. Queued-check recovery, the lower five exact threshold pairs, and the v0.22/non-AP/malformed-v0.23 compatibility matrix remain unverified.
