...
Status: **experimental Client v0.69.0 / APWorld v0.23.0 candidate partially live-verified but not accepted for release**. Offline verification and scoped final review passed. Live testing verified weighted receipts, the 180-point cap, exact threshold pairs at 5/10/20/64/89/111/140, all nine one-time chest checks, medal invariance, retained disconnected display, full-relaunch restoration, authoritative empty-history correction, and automatic reconnect/history rebuild. The project owner waived the redundant 32- and 46-point threshold pairs. The reviewed responsiveness, Game Garage entrance-preview, and final pre-login callback fixes were deployed as exact reviewed artifacts and passed their focused live regressions. Offline queued-check recovery and compatibility regressions remain incomplete. The native chest detour candidate is rejected and must not be deployed.
...
| Reviewed post-run reconnect/preview fix candidate DLL (`60987df`) | `99F3755D28917E9BF02E1296D4BA8468CF46845A499D04006490440B63233967` | Explicitly approved and deployed; installed hash matched; all 19 client test projects and `-SkipInstall` build passed; independent review found no issues; focused reconnect and Garage-preview live tests passed |
| Rejected pre-login observational-error candidate | `2B462A7A5A8D642A108FC5C23ABDBD0C71A7BE3458AB7F10FFF7C587B11125BF` | Deployed for focused regression and rejected after a connection-refused callback still stalled the active login attempt |
| Rejected eager socket-state candidate | `FA7B65091F701891254BF0FD9CD52A8145378F1FC5D1D893E228EEA356FB2C52` | Deployed for focused regression; ordinary timeouts advanced, but a later refusal-style attempt stalled because the pre-login callback still evaluated `session.Socket.Connected` before returning |
| Final lazy socket-state candidate | `E31D9916A807885F8CB5244769694A33514E713DDB35DDFC25B68E95568FF9E0` | Focused test observed RED before implementation and GREEN afterward; all 19 client test projects passed; `-SkipInstall` build completed with 0 errors and the same 7 existing nullable warnings; installed hash matched; live server-off retries completed at 1/2/5/5 seconds without a pre-login callback stall, then the same server restored automatically and rebuilt points from 0 to 0 without correction |
...
| Errors, exceptions, or mismatches | Initial reconnect observation ended during the prior 30-second backoff; a repeat diagnostic confirmed automatic recovery and exact history rebuild to 180. The reviewed follow-up build capped retries at five seconds and removed false Bloody Tears/Gradius entrance-preview entries; both fixes passed focused live regression. |
...
| Reconnect history rebuild, same total, queued check sent once | PARTIAL PASS — with the reviewed follow-up build, retry delays were observed at 1, 2, 5, 5 seconds, the game automatically reconnected after the same server returned, and history rebuilt from 180 to exactly 180 without correction or double-counting; queued-check recovery remains untested |
| Delayed/partial reconnect retains total; authoritative empty history corrects to zero | PASS — after preserving the 10-point server history, restarting the same seed with an empty authoritative history corrected the retained client total from 10 to zero. The final callback fix returns from pre-login errors without reading socket state. With the server stopped, four consecutive login attempts returned failure and advanced through 1/2/5/5-second waits while the game remained responsive; restarting the same server produced a second successful login, `reconnect rebuild previous=0 total=0 corrected=False`, and `NET reconnect succeeded`. |
...
| Errors and log/video references | Client log above. The reviewed follow-up build reduced the retry maximum from 30 to five seconds and passed live reconnect regression. The adjacent Garage entrance-preview false positive also passed live regression: only the native Vampire Killer cartridge appeared, while AP-unowned Bloody Tears and Gradius remained absent. No crash or point-total corruption observed during this run. |
...
| ---: | --- | ---: | --- | --- | --- | --- |
| 5 | Music Lab - 5 Point Chest | 187256169 | 4: PASS | 5: PASS | PASS | `187256169`; once |
| 10 | Music Lab - 10 Point Chest | 187256170 | 9: PASS | 10: PASS | PASS | `187256170`; once |
| 20 | Music Lab - 20 Point Chest | 187256171 | 19: PASS | 20: PASS | PASS | `187256171`; once |
| 32 | Music Lab - 32 Point Chest | 187256172 | WAIVED | WAIVED | PASS | `187256172`; once — exact below/at pair waived by project owner after representative lower and upper thresholds passed |
| 46 | Music Lab - 46 Point Chest | 187256173 | WAIVED | WAIVED | PASS | `187256173`; once — exact below/at pair waived by project owner after representative lower and upper thresholds passed |
...
**NOT ACCEPTED — remaining targeted regression required.** The reviewed room-scoped implementation passed offline verification and the core weighted-point, cap, representative lower and upper threshold, one-time-check, medal-invariance, disconnected-display, authoritative-empty correction, automatic-reconnect/history-rebuild, and full-relaunch tests. Exact below/at tests passed at 5, 10, 20, 64, 89, 111, and 140 points; the project owner waived the redundant 32- and 46-point pairs. Reviewed commit `60987df` reduced the maximum reconnect wait to five seconds while preserving infinite retries and stale-worker protection. A later reset exposed a distinct connection-refused callback stall. The final follow-up fix makes socket-state evaluation lazy so pre-login errors return without touching the socket; its exact deployed DLL completed repeated server-off retries, automatically reconnected when the same server returned, and rebuilt the authoritative zero-point history without correction or duplication. Queued-check recovery and the v0.22/non-AP/malformed-v0.23 compatibility matrix remain unverified.

An adjacent defect was discovered during the same run: the Game Garage entrance preview treated native source-collected flags as ownership for randomized cartridges. Bloody Tears and Gradius appeared in the entrance preview even though the player had not received them through AP and the actual Garage correctly kept them unplayable. The follow-up build makes AP ownership authoritative for the five randomized preview entries while retaining native behavior for disabled/incompatible sessions and Vampire Killer. Focused live regression passed: the entrance preview displayed Vampire Killer alone and no longer showed the two AP-unowned cartridges.
