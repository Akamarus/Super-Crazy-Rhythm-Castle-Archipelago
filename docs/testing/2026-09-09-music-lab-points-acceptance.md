...
Status: **experimental Client v0.69.0 / APWorld v0.23.0 candidate partially live-verified but not accepted for release**. Offline verification and scoped final review passed. Live testing verified weighted receipts, the 180-point cap, the upper four chest thresholds, all nine one-time chest checks, medal invariance, retained disconnected display, full-relaunch restoration, and automatic reconnect/history rebuild. A repeat diagnostic showed that the earlier reconnect failure was a 30-second backoff delay rather than a stopped worker. A reviewed responsiveness change and fix for the separate Game Garage entrance-preview false positive now pass offline regression tests but have not been deployed or live-tested. The lower five threshold pairs, offline queued-check recovery, and compatibility regressions remain incomplete. The native chest detour candidate is rejected and must not be deployed.
...
| Reviewed post-run reconnect/preview fix candidate DLL (`60987df`) | `99F3755D28917E9BF02E1296D4BA8468CF46845A499D04006490440B63233967` | Offline only; all 19 client test projects and `-SkipInstall` build passed; independent review found no issues; not approved for deployment or live-tested |
...
**NOT ACCEPTED — targeted deployment and live regression required.** The reviewed room-scoped implementation passed offline verification and the core weighted-point, cap, upper-threshold, one-time-check, medal-invariance, disconnected-display, automatic-reconnect/history-rebuild, and full-relaunch tests. The repeat reconnect diagnostic proved that the worker had not stopped; it was waiting at its 30-second maximum backoff. Reviewed commit `60987df` reduces that maximum wait to five seconds while preserving infinite retries and stale-worker protection, but the new artifact is not yet deployed or live-tested. Queued-check recovery, the lower five exact threshold pairs, and the v0.22/non-AP/malformed-v0.23 compatibility matrix remain unverified.
