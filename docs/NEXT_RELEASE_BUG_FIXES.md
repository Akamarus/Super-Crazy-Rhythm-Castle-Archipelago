...
- [ ] **Reconnect after an Archipelago server restart or dropped socket.** The client currently logs the disconnect but does not reconnect automatically, and reloading the current hub does not recover the connection. Until fixed, a full game relaunch is required. This is a real client robustness defect, but it is not a seed-logic failure and is lower priority than progression blockers.
