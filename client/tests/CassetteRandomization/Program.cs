...
var gatedState = new DiagnosticSaveState
{
    CurrentStatus = DiagnosticCassetteStatus.HAVE_NOT_EARNED,
    PredicateResult = true,
    PredicateStatus = DiagnosticCassetteStatus.HAVE_NOT_EARNED
};
var gatedProcessor = new DiagnosticProcessor(gatedState);
var diagnosticRequest = new DiagnosticCassetteRequest(
    DiagnosticSong.QUIERES_BAILAR,
    DiagnosticCassetteStatus.HAVE_IN_BAG,
    bundle: null);
Equal<CassetteBundleCollisionPatchState?>(null,
    CassetteBundleCollisionDiagnostic.Begin(false, gatedProcessor, diagnosticRequest),
    "vanilla-origin request does not enter diagnostic capture");
Equal(0, gatedProcessor.ObtainStateCalls, "AP-only gate prevents processor state lookup");
Equal(0, gatedState.PredicateCalls, "AP-only gate prevents cross-bundle predicate call");
Equal(0, gatedState.StatusReads, "AP-only gate prevents cassette status reads");

var collisionState = new DiagnosticSaveState
{
    CurrentStatus = DiagnosticCassetteStatus.HAVE_NOT_EARNED,
    PredicateResult = true,
    PredicateStatus = DiagnosticCassetteStatus.HAVE_NOT_EARNED
};
CurrentPlayerSaveEnquiries.SelectedState = collisionState;
var collisionCapture = CassetteBundleCollisionDiagnostic.Begin(
    true, new DiagnosticProcessor(collisionState), diagnosticRequest);
Equal(true, collisionCapture != null, "AP-origin request begins reflective capture");
Equal(1, collisionState.PredicateCalls, "capture invokes the cross-bundle predicate exactly once");
Equal(DiagnosticBundle.DEFAULT, collisionState.ExcludedBundle, "null request bundle excludes the effective DEFAULT bundle");
Equal(1, collisionState.StatusReads, "prefix capture reads native status once before the request");
string collisionCaptureLine = CassetteBundleCollisionDiagnostic.Complete(collisionCapture)!;
Equal(1, collisionState.PredicateCalls, "postfix does not repeat the predicate");
Equal(2, collisionState.StatusReads, "postfix reads native status exactly once after the request");
Equal(true, collisionCaptureLine.Contains("outcome='cross-bundle-rejection-indicated'", StringComparison.Ordinal), "reflective capture reports collision indication");

var falsifiedState = new DiagnosticSaveState
{
    CurrentStatus = DiagnosticCassetteStatus.HAVE_NOT_EARNED,
    PredicateResult = false,
    PredicateStatus = null
};
CurrentPlayerSaveEnquiries.SelectedState = falsifiedState;
var falsifiedCapture = CassetteBundleCollisionDiagnostic.Begin(
    true, new DiagnosticProcessor(falsifiedState), diagnosticRequest);
string falsifiedCaptureLine = CassetteBundleCollisionDiagnostic.Complete(falsifiedCapture)!;
Equal(1, falsifiedState.PredicateCalls, "falsification capture invokes predicate once");
Equal(2, falsifiedState.StatusReads, "falsification capture brackets the request with two status reads");
Equal(true, falsifiedCaptureLine.Contains("outcome='predicate-false-unchanged'", StringComparison.Ordinal), "reflective capture reports predicate falsification");

var stagedState = new DiagnosticSaveState
{
    CurrentStatus = DiagnosticCassetteStatus.HAVE_NOT_EARNED,
    PredicateResult = false,
    PredicateStatus = null
};
CurrentPlayerSaveEnquiries.SelectedState = stagedState;
var stagedCapture = CassetteBundleCollisionDiagnostic.Begin(
    true, new DiagnosticProcessor(stagedState), diagnosticRequest);
Equal(1, stagedState.StatusReads, "stage capture reads status before existing request");
stagedState.CurrentStatus = DiagnosticCassetteStatus.HAVE_IN_BAG;
string stagedCaptureLine = CassetteBundleCollisionDiagnostic.Complete(stagedCapture)!;
Equal(1, stagedState.PredicateCalls, "stage capture invokes predicate once");
Equal(2, stagedState.StatusReads, "stage capture reads status after existing request");
Equal(true, stagedCaptureLine.Contains("outcome='stage-observed'", StringComparison.Ordinal), "reflective capture observes AddChange-equivalent status transition");

string pluginSource = File.ReadAllText(Path.Combine(Directory.GetCurrentDirectory(), "client", "Plugin.cs"));
string cassetteHook = ExtractMethods(pluginSource, "private int PatchCassetteStatusRequest()").Single();
Equal(true, cassetteHook.Contains("prefix: new HarmonyMethod(prefix)", StringComparison.Ordinal), "exact cassette overload installs diagnostic prefix");
Equal(true, cassetteHook.Contains("postfix: new HarmonyMethod(postfix)", StringComparison.Ordinal), "exact cassette overload installs diagnostic postfix");
string cassettePrefix = ExtractMethods(pluginSource, "public static bool CassetteStatusRequestPrefix(").Single();
Equal(true, cassettePrefix.Contains("out CassetteBundleCollisionPatchState? __state", StringComparison.Ordinal), "cassette prefix carries Harmony state");
Equal(true, cassettePrefix.Contains("CassetteReceiptRandomization.IsApplyingNativeGrant", StringComparison.Ordinal), "cassette prefix supplies the AP-only gate");
Equal(true, cassettePrefix.Contains("CassetteBundleCollisionDiagnostic.Begin(", StringComparison.Ordinal), "cassette prefix starts production reflection capture");
string cassettePostfix = ExtractMethods(pluginSource, "public static void CassetteStatusRequestPostfix(").Single();
Equal(true, cassettePostfix.Contains("CassetteBundleCollisionDiagnostic.Complete(__state)", StringComparison.Ordinal), "cassette postfix completes the same Harmony state");
...
enum DiagnosticSong { INVALID, QUIERES_BAILAR }
enum DiagnosticCassetteStatus { INVALID, HAVE_IN_BAG, HAVE_DEPOSITED, HAVE_NOT_EARNED }
enum DiagnosticBundle { INVALID, DEFAULT, OTHER }

sealed class DiagnosticCassetteRequest
{
    public DiagnosticSong Song { get; }
    public DiagnosticCassetteStatus CassetteStatus { get; }
    public DiagnosticBundle? Bundle { get; }

    public DiagnosticCassetteRequest(
        DiagnosticSong song,
        DiagnosticCassetteStatus cassetteStatus,
...
sealed class DiagnosticProcessor
{
    private readonly DiagnosticSaveState _state;
    public int ObtainStateCalls { get; private set; }

    public DiagnosticProcessor(DiagnosticSaveState state) => _state = state;

    private DiagnosticSaveState ObtainState()
    {
        ObtainStateCalls++;
        return _state;
    }
...
sealed class DiagnosticSaveState
{
    public IntPtr Pointer { get; } = new(0x1111);
    public DiagnosticCassetteStatus CurrentStatus { get; set; }
    public bool PredicateResult { get; set; }
    public DiagnosticCassetteStatus? PredicateStatus { get; set; }
    public DiagnosticBundle ExcludedBundle { get; private set; }
    public int PredicateCalls { get; private set; }
    public int StatusReads { get; private set; }

    public DiagnosticCassetteStatus GetCassetteStatusForSong(DiagnosticSong song)
    {
        StatusReads++;
        return CurrentStatus;
    }

    private bool HasSongCassetteStatusBeenChangedByAnyBundle(
        DiagnosticSong song,
        out DiagnosticCassetteStatus? status,
        DiagnosticBundle? bundleToExclude)
    {
        PredicateCalls++;
        ExcludedBundle = bundleToExclude ?? DiagnosticBundle.INVALID;
        status = PredicateStatus;
        return PredicateResult;
    }
}

static class CurrentPlayerSaveEnquiries
{
    internal static object? SelectedState { get; set; }

    private static object? GetSelectedSaveFileState() => SelectedState;
}
