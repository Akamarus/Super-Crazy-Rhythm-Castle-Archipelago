...
internal sealed class ApStarSessionHistory
{
    ...
    private bool _configured, _historyReady;
    ...
    internal ApStarSettings ApplySlotData(Dictionary<string,object>? data, string identity)
    {
        lock (_sync)
        {
            ...
            _state.Configure(_generation,identity,settings);_configured=true;Publish();return settings;
        }
    }
    internal void HandlePacket(ArchipelagoPacketBase packet)
    {
        ...
        lock(_sync)
        {
            if(received.Items==null||received.Index<0){_historyReady=false;return;}
            if(received.Index==0){_history.Clear();_historyReady=true;}
            else if(!_historyReady||received.Index!=_history.Count){_historyReady=false;return;}
