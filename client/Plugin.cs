...
internal static class GamePatches
{
        ...
        out CassetteBundleCollisionPatchState? __state)
    {
        ...
        __state = CassetteBundleCollisionDiagnostic.Begin(
            CassetteReceiptRandomization.IsApplyingNativeGrant,
            __instance,
            request);
        if (request == null)
            return true;
    ...
    {
        string? diagnostic = CassetteBundleCollisionDiagnostic.Complete(__state);
        if (diagnostic != null)
            Plugin.LoggerInstance?.LogWarning(diagnostic);
    }
